...
byte libaroma_listitem_nonitem(LIBAROMA_CTL_LIST_ITEMP item){
  ...
  return libaroma_listitem_iscaption(item) || libaroma_listitem_istext(item);
} /* End of libaroma_listitem_nonitem */

/*
 * Function    : libaroma_ctl_list_redraw_item
 * Return Value: void
 * Descriptions: redraw item by id
 */
void libaroma_ctl_list_redraw_item(LIBAROMA_CONTROLP ctl, byte item_id) {
LIBAROMA_CTL_LIST_ITEMP itemp = libaroma_ctl_list_get_item_internal(ctl, item_id, 1);
_libaroma_ctl_list_dodraw_item(ctl, itemp);
}
