...
static LIBAROMA_THREAD _libaroma_wm_message_handling_thread_var;
static LIBAROMA_THREAD _libaroma_wm_device_info_thread_var;
 ...
 * Descriptions: set message handler
 */
byte libaroma_wm_set_message_thread_handler(
    LIBAROMA_WMCB_UI_THREAD callback) {
  if (_libaroma_wm==NULL){
    ALOGW("window manager uninitialized");
    return 0;
  }
  _libaroma_wm->message_thread_handler = callback;
  return 1;
} /* End of libaroma_wm_set_message_thread_handler */

/*
 * Function    : libaroma_wm_set_deviceinfo_handler
 * Return Value: byte
 * Descriptions: set device info handler
 */
byte libaroma_wm_set_deviceinfo_handler(
    LIBAROMA_WMCB_UI_THREAD callback) {
  if (_libaroma_wm==NULL){
    ALOGW("window manager uninitialized");
    return 0;
  }
  _libaroma_wm->deviceinfo_thread_handler = callback;
  return 1;
} /* End of libaroma_wm_set_deviceinfo_handler */
 ...
 * Descriptions: set message handler
 */
byte libaroma_wm_set_power_menu_window_handler(
    LIBAROMA_WMCB_UI_THREAD callback) {
  if (_libaroma_wm==NULL){
    ALOGW("window manager uninitialized");
    return 0;
  }
  _libaroma_wm->power_menu_window = callback;
  return 1;
} /* End of libaroma_wm_set_power_menu_window_handler */
...
/*
 * Function    : _libaroma_wm_message_handling_thread
 * Return Value: static void *
 * Descriptions: window manager custom message handler
 */
static void * _libaroma_wm_message_handling_thread(void * cookie) {
  ALOGV("starting wm message handling thread");
  while(_libaroma_wm->client_started){
  if (_libaroma_wm->message_thread_handler!=NULL){
       _libaroma_wm->message_thread_handler();
   }
   libaroma_sleep(80);
 }
  ALOGV("wm message handling thread ended");
  return NULL;
} /* End of _libaroma_wm_message_handling_thread */
 ...
 * Descriptions: window manager custom message handler
 */
static void * _libaroma_wm_deviceinfo_handling_thread(void * cookie) {
  ALOGV("starting wm device info handling thread");
  while(_libaroma_wm->client_started){
  if (_libaroma_wm->deviceinfo_thread_handler!=NULL){
       _libaroma_wm->deviceinfo_thread_handler();
   }
   libaroma_sleep(80);
 }
  ALOGV("wm device info handling thread ended");
  return NULL;
} /* End of _libaroma_wm_message_handling_thread */
...
byte libaroma_wm_client_start(){
  ...
  /* start message handling thread */
    libaroma_thread_create(&_libaroma_wm_message_handling_thread_var,
      _libaroma_wm_message_handling_thread, NULL);
      
      /* start device info thread */
    libaroma_thread_create(&_libaroma_wm_device_info_thread_var,
      _libaroma_wm_deviceinfo_handling_thread, NULL);
...
byte libaroma_wm_client_stop(){
  ...
  if (_libaroma_wm->client_started){
    ...
    libaroma_thread_join(_libaroma_wm_message_handling_thread_var);
    libaroma_thread_join(_libaroma_wm_device_info_thread_var);
    _libaroma_wm_message_thread_var=0;
    _libaroma_wm_ui_thread_var=0;
    _libaroma_wm_message_handling_thread_var=0;
    _libaroma_wm_device_info_thread_var=0;
 ...
 * Return Value: byte
 * Descriptions: set current active window
 */
...
/*
 * Function    : libaroma_wm_sync_active_window
 * Return Value: void
 * Descriptions: sync current active window
 */
void libaroma_wm_sync_active_window(){
  if (_libaroma_wm==NULL){
    ALOGW("window manager uninitialized");
    return;
  }
 if (_libaroma_wm->active_window != NULL) {
  if (_libaroma_wm->active_window->handler!=NULL) {
    if (_libaroma_wm->active_window->handler->invalidate!=NULL){
      _libaroma_wm->active_window->handler->invalidate(_libaroma_wm->active_window, 1);
     }
   }   
 }
} /* End of libaroma_wm_sync_active_window */



/*
 * Function    : libaroma_window_send_event
 * Return Value: byte
 * Descriptions: send event to the currently active window
 */
byte libaroma_window_send_event(byte event){
  if (_libaroma_wm==NULL){
    ALOGW("window manager uninitialized");
    return 0;
  }
  libaroma_msg_post(event,0,0,0,0,NULL);
  return 1;
} /* End of libaroma_wm_set_workspace */
