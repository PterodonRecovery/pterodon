...
void libaroma_dialog_message(
  ...
  word title_color = libaroma_colorget(NULL, win)->dialog_secondary_text;
  word msg_color = libaroma_colorget(NULL, win)->dialog_primary_text;
  word btncolor = libaroma_colorget(NULL, win)->dialog_primary_text;
  byte button_style = 0;
  if (flags&LIBAROMA_DIALOG_ACCENT_BUTTON){
    btncolor = libaroma_colorget(NULL, win)->accent;
  ...
  do{
    ...
    if (msg.msg==LIBAROMA_MSG_KEY_BACK){
      if (flags&LIBAROMA_DIALOG_CANCELABLE){
        ALOGV("libaroma_dialog_message: Back Button - Cancel");
        onpool=0;
      }
    }
    else if (cmd==LIBAROMA_CMD_CLICK){
      if (id==1){
        ALOGV("libaroma_dialog_message: Button 1 Selected");
        onpool=0;
        //libaroma_sleep(300);
      }
    }
      ...
      if (flags&LIBAROMA_DIALOG_CANCELABLE){
        if (msg.state==1){
          if ((msg.x<cdata->x)||(msg.y<cdata->y)||(msg.y>cdata->y+cdata->h)||
            (msg.x>cdata->x+cdata->w)){
            ALOGV("libaroma_dialog_message: Touch Outside Dialog - Cancel");
            onpool=0;
          }
        }
      }
...
int libaroma_dialog_list(
  ...
  int u = 0;
  byte first_option = 1;
  const char* option_arg = options;
  va_list ap;
  va_start(ap, options);
  while (1) {
  if (first_option == 0) {
  option_arg = va_arg(ap, const char*);
  if (!option_arg) break;
  }
    ...
    if ((selected_option < 0 && first_option == 1) || selected_option == u){
